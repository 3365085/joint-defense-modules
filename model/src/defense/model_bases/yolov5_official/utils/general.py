...
try:
    ...
    raise RuntimeError("Bundled YOLOv5 base is offline-only; ultralytics must be preinstalled.")

from ultralytics.utils.checks import check_requirements
from ultralytics.utils.patches import torch_load

from utils import TryExcept, emojis
from utils.downloads import OFFLINE_ONLY_MESSAGE
...
def check_online():
    """Network checks are disabled in the bundled runtime base."""
    return False
...
def check_git_status(repo="ultralytics/yolov5", branch="master"):
    """Network update checks are disabled in the bundled runtime base."""
    LOGGER.info(colorstr("github: ") + "skipping check (offline-only bundled base)")


@WorkingDirectory(ROOT)
def check_git_info(path="."):
    """Return empty git metadata for the bundled runtime base."""
...
def check_version(current="0.0.0", minimum="0.0.0", name="version ", pinned=False, hard=False, verbose=False):
    ...
    s = f"WARNING 鈿狅笍 {name}{minimum} is required by YOLOv5, but {name}{current} is currently installed"  # string
    if hard:
...
def check_img_size(imgsz, s=32, floor=0):
    ...
    if new_size != imgsz:
        LOGGER.warning(f"WARNING 鈿狅笍 --img-size {imgsz} must be multiple of max stride {s}, updating to {new_size}")
    return new_size
...
def check_imshow(warn=False):
    ...
    try:
        ...
        if warn:
            LOGGER.warning(f"WARNING 鈿狅笍 Environment does not support cv2.imshow() or PIL Image.show()\n{e}")
        return False
...
def check_font(font=FONT, progress=False):
    ...
    if not font.exists() and not file.exists():
        raise FileNotFoundError(f"{OFFLINE_ONLY_MESSAGE} Missing local font file: {font.name}")


def check_dataset(data, autodownload=True):
    """Dataset validation is disabled in the bundled runtime base."""
    raise RuntimeError(OFFLINE_ONLY_MESSAGE)


def check_amp(model):
    """Skip AMP self-tests in the bundled runtime base to avoid remote fallback models."""
    return False
...
def download(url, dir=".", unzip=True, delete=True, curl=False, threads=1, retry=3):
    """Reject downloads in the bundled runtime base."""
    raise RuntimeError(OFFLINE_ONLY_MESSAGE)
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if (time.time() - t) > time_limit:
            LOGGER.warning(f"WARNING 鈿狅笍 NMS time limit {time_limit:.3f}s exceeded")
