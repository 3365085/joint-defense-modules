...
# Import 'ultralytics' package. The bundled base is local-only and must not install packages at runtime.
try:
    import ultralytics

    assert hasattr(ultralytics, "__version__")  # verify package is not directory
except (ImportError, AssertionError):
    raise RuntimeError("Bundled YOLOv5 base requires preinstalled ultralytics; runtime installation is disabled.")
            ...
            for i, im in enumerate(ims):
                ...
                if isinstance(im, (str, Path)):  # filename or uri
                    if str(im).startswith("http"):
                        raise RuntimeError("Bundled YOLOv5 base is offline-only; remote image URLs are disabled.")
