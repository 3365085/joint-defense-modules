...
def test_symmetric_brightness_motion_does_not_trigger_temporal_flash(
    cuda_device: str,
) -> None:
    det = GPUOverexposureDetector(threshold=0.90)
    base = torch.full((64, 64), 100.0)
    changed = base.clone()
    changed[:, :32] = 140.0
    changed[:, 32:] = 60.0

    det.compute(_gray(base, cuda_device))
    out = det.compute(_gray(changed, cuda_device))

    assert out["temporal_flash_ratio"] == pytest.approx(1.0)
    assert out["temporal_flash_polarity"] == pytest.approx(0.0)
    assert out["temporal_flash"] is False
    assert out["is_glare"] is False


def test_one_way_brightness_jump_triggers_temporal_flash(cuda_device: str) -> None:
    det = GPUOverexposureDetector(threshold=0.90)
    base = torch.full((64, 64), 100.0)
    changed = torch.full((64, 64), 140.0)

    det.compute(_gray(base, cuda_device))
    out = det.compute(_gray(changed, cuda_device))

    assert out["temporal_flash_bright_ratio"] == pytest.approx(1.0)
    assert out["temporal_flash_dark_ratio"] == pytest.approx(0.0)
    assert out["temporal_flash_polarity"] == pytest.approx(1.0)
    assert out["temporal_flash"] is True
    assert out["is_glare"] is True
