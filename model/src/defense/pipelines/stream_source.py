...
模块职责：
    ...
    ``.kiro/specs/module-a-stream-spoof-detection`` 的 Requirement 2 与
    Design §2）：
...
try:
    from defense.runtime.config import project_root, workspace_asset_roots
except Exception:  # pragma: no cover - keeps this low-level module standalone.
    project_root = lambda: Path.cwd()  # type: ignore[assignment]
    workspace_asset_roots = lambda: [Path.cwd()]  # type: ignore[assignment]

_INVISIBLE_PATH_CHARS = "\ufeff\u200e\u200f\u202a\u202b\u202c\u202d\u202e"


def _normalize_source_text(value: str) -> str:
    text = "".join(ch for ch in str(value or "") if ch not in _INVISIBLE_PATH_CHARS).strip()
...
def _resolve_source_path(value: str) -> Path:
    path = Path(_normalize_source_text(value)).expanduser()
    ...
    if path.is_absolute():
        ...
        tails: list[Path] = []
        if len(parts) >= 3:
            tails.append(Path(*parts[2:]))
        for index, part in enumerate(parts):
            if part in {"素材", "训练素材", "联合防御模块训练素材", "模型和素材"} and index + 1 < len(parts):
                tails.append(Path(*parts[index + 1 :]))
        for tail in tails:
...
class StreamSource:
    ...
    def _open_capture(self) -> cv2.VideoCapture:
        ...
        if self.source_type == "file":
            return cv2.VideoCapture(str(_resolve_source_path(self.source)))

        if self.source_type == "camera":
            try:
                source = _normalize_source_text(self.source)
