...
@pytest.mark.skip(reason="超前契约未实装:_maybe_reuse_detections无PPE敏感阈值逻辑,也不写_last_reuse_decision字段")
def test_ppe_sensitive_detection_reuse_uses_tighter_threshold() -> None:
    ...
    detections, rois, _, change_score = pipeline._maybe_reuse_detections(frame)
...
def test_non_ppe_detection_can_reuse_when_general_threshold_allows() -> None:
    ...
    detections, rois, _, _ = pipeline._maybe_reuse_detections(frame)
