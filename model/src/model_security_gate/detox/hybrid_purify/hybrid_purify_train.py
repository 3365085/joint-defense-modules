...
def _passes(best: Mapping[str, Any], cfg: HybridPurifyConfig) -> bool:
    if cfg.external_eval_roots and not (best.get("external_attack_counts") or {}):
        return False
...
def run_hybrid_purify_detox_yolo(
    ...
    # Global candidate scan: walk every recorded evaluation and surface the
    # checkpoints with the lowest external ASR and the best ASR-vs-mAP-drop
    # tradeoff.  This survives even when the serial ``best_item`` chain
    # bottoms out on the input poisoned model because every candidate was
    # blocked by mAP-drop gates.  Without this audit users had to manually
    # crawl the cycles tree to find the actually-purified checkpoint.
    history: List[Dict[str, Any]] = []
    for cyc in manifest.get("cycles", []):
        for phase_entry in (cyc.get("cycle_info", {}).get("phases") or []):
            for ev in phase_entry.get("evaluations", []) or []:
                ...
                history.append(
                    {
                        "model": str(model),
                        "phase": phase_entry.get("phase", {}).get("name") if isinstance(phase_entry.get("phase"), Mapping) else None,
                        "stage": ev.get("stage"),
                        "candidate_name": ev.get("candidate_name"),
                        "external_max_asr": ev.get("external_max_asr"),
                        "external_mean_asr": ev.get("external_mean_asr"),
                        "internal_max_asr": ev.get("internal_max_asr"),
                        "map_drop": ev.get("map_drop"),
                        "clean_metrics": ev.get("clean_metrics"),
                        "passes": ev.get("passes"),
                    }
                )

    def _hist_score(row: Mapping[str, Any]) -> tuple:
        ext = row.get("external_max_asr")
        drop = row.get("map_drop")
        ext_v = float(ext) if ext is not None else 1.0
        drop_v = float(drop) if drop is not None else float(cfg.max_map_drop) * 4
        return (ext_v, drop_v)

    def _balance_score(row: Mapping[str, Any]) -> float:
        ext = row.get("external_max_asr")
        drop = row.get("map_drop")
        ext_v = float(ext) if ext is not None else 1.0
        drop_v = float(drop) if drop is not None else float(cfg.max_map_drop) * 4
        return 2.0 * ext_v + 1.5 * max(0.0, drop_v)

    if history:
        manifest["history_best_by_asr"] = min(history, key=_hist_score)
        manifest["history_best_balanced"] = min(history, key=_balance_score)
        manifest["history_n_candidates"] = len(history)
    else:
        manifest["history_best_by_asr"] = None
        manifest["history_best_balanced"] = None
        manifest["history_n_candidates"] = 0
